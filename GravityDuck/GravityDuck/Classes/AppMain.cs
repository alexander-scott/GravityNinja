		...
		public static void CheckInput()
		{
			...
			foreach(TouchData data in touches)
			{
				...
				{				
					...
					if((oldTouchPos.Y - newTouchPos.Y) < -0.30f)					
						cameraRotation -= FMath.PI;	
			...
			if(play)
			{
				...
				if(FMath.Cos(cameraRotation) < ((-FMath.PI/4)))
					cameraRotation = FMath.Acos(-FMath.PI/4);
				else if(FMath.Cos(cameraRotation) > ((FMath.PI/4)))
						cameraRotation = FMath.Acos(FMath.PI/4);		
