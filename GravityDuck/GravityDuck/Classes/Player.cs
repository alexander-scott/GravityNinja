...
namespace GravityDuck
{
	...
	public class Player
	{
		...
		{	
			...
			sprite.Position = new Vector2(-100.0f, 330.0f); //Starting position (will be changed)
		...
		public void Update(Vector2 gravity, float gravityAngle)
		{	        
			// Rotate the player towards the direction of gravity.		RMDS	
			//float gravityAngle = FMath.Atan(gravity.X / gravity.Y);
			      
			//if(gravity.X == 0 || gravity.Y == 0)
				//gravityAngle = 0.0f;
			
			if(sprite.Angle != gravityAngle)
				if(sprite.Angle < gravityAngle)
				{
					sprite.Angle += 0.01f;
				}
			...
			if(gravVelocity < 0.3f) //If not falling
			{
				...
				sprite.Position = new Vector2(sprite.Position.X + ((directionVector.X * tempDir.X) * velocity),
				                                     sprite.Position.Y + ((directionVector.Y * tempDir.Y) * velocity));
			}
			else //Else falling
			{
				...
				if(velocity > 0.0f) //Decrease the movement velocity so it doesn't immediatley stop when it comes to an edge
					velocity -= speed/2;
				sprite.Position = new Vector2(sprite.Position.X + (gravity.X * gravVelocity) + (directionVector.X * velocity),
				                                     sprite.Position.Y + (gravity.Y * gravVelocity) + (directionVector.Y * velocity));
			}
		}                  
